...
class ChapterRepository(BaseRepository[Chapter]):
    ...
    async def get_with_contents(self, chapter_id: uuid.UUID) -> Chapter | None:
        """获取章节及其内容"""
        stmt = select(Chapter).options(selectinload(Chapter.contents)).where(Chapter.id == chapter_id)
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()

    async def get_with_sections(self, chapter_id: uuid.UUID) -> Chapter | None:
        """获取章节及其小节"""
        stmt = select(Chapter).options(selectinload(Chapter.sections)).where(Chapter.id == chapter_id)
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()

    async def get_with_reviews(self, chapter_id: uuid.UUID) -> Chapter | None:
        """获取章节及其审核记录"""
        stmt = select(Chapter).options(selectinload(Chapter.reviews)).where(Chapter.id == chapter_id)
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def update_content_hash(self, chapter_id: uuid.UUID, content_hash: str) -> Chapter | None:
        """更新内容哈希"""
        return await self.update(chapter_id, content_hash=content_hash)

    async def update_word_count(self, chapter_id: uuid.UUID, word_count: int) -> Chapter | None:
        """更新字数统计"""
        return await self.update(chapter_id, word_count=word_count)

    async def get_by_parent(self, parent_chapter_id: uuid.UUID) -> Sequence[Chapter]:
        """获取子章节"""
    ...
    async def get_next_order_num(self, project_id: uuid.UUID) -> int:
        """获取项目下一个可用的章节序号"""
        stmt = select(func.max(Chapter.order_num)).where(Chapter.project_id == project_id)
        result = await self._session.execute(stmt)
        max_order = result.scalar_one_or_none()
        return (max_order or 0) + 1

    async def reorder_chapters(self, project_id: uuid.UUID, chapter_orders: list[dict[str, Any]]) -> None:
        """批量重排章节顺序 - 使用事务包装"""
...
class ChapterContentRepository(BaseRepository[ChapterContent]):
    ...
    async def get_by_chapter(self, chapter_id: uuid.UUID) -> Sequence[ChapterContent]:
        """获取章节的所有内容版本"""
        return await self.find_all(
    ...
    async def get_by_hash(self, chapter_id: uuid.UUID, content_hash: str) -> ChapterContent | None:
        """根据内容哈希获取内容"""
...
class SectionRepository(BaseRepository[Section]):
    ...
    async def get_by_chapter(self, chapter_id: uuid.UUID, *, status: str | None = None) -> Sequence[Section]:
        """获取章节下的所有小节"""
        filters = {"chapter_id": chapter_id}
        if status:
            filters["status"] = status
    ...
    async def get_with_parent(self, section_id: uuid.UUID) -> Section | None:
        """获取小节及其父小节"""
        stmt = select(Section).options(joinedload(Section.parent_section)).where(Section.id == section_id)
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()

    async def get_child_sections(self, parent_section_id: uuid.UUID) -> Sequence[Section]:
        """获取子小节"""
        return await self.find_all(
    ...
    async def get_next_order_num(self, chapter_id: uuid.UUID) -> int:
        """获取章节下一个小节的序号"""
        stmt = select(func.max(Section.order_num)).where(Section.chapter_id == chapter_id)
        result = await self._session.execute(stmt)
        max_order = result.scalar_one_or_none()
        return (max_order or 0) + 1
