...
class ContentGenerationActivity:
        ...
        try:
            ...
            for section in sections:
                ...
                for subsection in section.get("subsections", []):
                    subsection_title = (
                        subsection if isinstance(subsection, str) else subsection.get("title", "Untitled")
                    )
                    total_content += f"### {subsection_title}\n\n"

                    content_block = await ContentGenerationActivity._generate_subsection_content(
                        input_data.chapter_id, section_title, subsection_title, input_data.difficulty_level
                    )
            ...
            activity.logger.info(f"Content generation completed for {input_data.chapter_id}: {word_count} words")
    ...
    async def _generate_subsection_content(
        chapter_id: str, section_title: str, subsection_title: str, difficulty: str
    ) -> str:
        """
        生成小节内容 (模拟AI生成)
    ...
    async def revise_chapter_content(chapter_id: str, original_content: str, revision_notes: list[str]) -> str:
        """
        根据审核意见修订内容
        ...
        Returns:
            str: 修订后的内容
        """
        activity.logger.info(f"Revising content for chapter: {chapter_id}, notes: {len(revision_notes)}")
    ...
    async def expand_chapter_content(chapter_id: str, current_content: str, target_word_count: int) -> str:
        """
        扩展内容以达到目标字数
        ...
        activity.logger.info(f"Expanding chapter {chapter_id}: {current_words} -> {target_word_count} words")
