...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("role_id", UUID(as_uuid=True), sa.ForeignKey("roles.id", ondelete="CASCADE"), primary_key=True),
        sa.Column(
            "permission_id", UUID(as_uuid=True), sa.ForeignKey("permissions.id", ondelete="CASCADE"), primary_key=True
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column("parent_chapter_id", UUID(as_uuid=True), sa.ForeignKey("chapters.id"), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")),
    )
    ...
    op.create_table(
        ...
        sa.Column("status", sa.String(20), server_default="draft"),
        sa.Column("word_count", sa.Integer(), server_default="0"),
        sa.Column("parent_section_id", UUID(as_uuid=True), sa.ForeignKey("sections.id"), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")),
    )
    ...
    op.create_table(
        ...
        sa.Column("source_chapter_id", UUID(as_uuid=True), sa.ForeignKey("chapters.id"), nullable=True),
    ...
    op.create_index("idx_graph_nodes_properties", "graph_nodes", [op.text("properties")], postgresql_using="gin")

    # 图谱边表
    op.create_table(
        "graph_edges",
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("source_id", sa.String(255), sa.ForeignKey("graph_nodes.id", ondelete="CASCADE"), nullable=False),
        sa.Column("target_id", sa.String(255), sa.ForeignKey("graph_nodes.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("idx_graph_edges_properties", "graph_edges", [op.text("properties")], postgresql_using="gin")
