...
class ExternalReviewService:
    ...
    def initiate_review(self, workflow_id: str, task_id: str, content: str, reviewer_id: str) -> ReviewSession:
        """发起审核会话"""
    ...
    def submit_review(self, session_id: str, result: str, comments: str = ""):
        """提交审核结果"""
    ...
    def _sign_review(self, session: ReviewSession, result: str, timestamp: datetime) -> str:
        ...
        if self.hsm_client:
            payload = (
                f"{session.workflow_id}|{session.task_id}|{content_hash}|{session.reviewer_id}|{timestamp.isoformat()}"
            )
