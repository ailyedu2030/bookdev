...
class ProjectRepository(BaseRepository[Project]):
    ...
    async def get_with_owner(self, project_id: uuid.UUID) -> Project | None:
        """获取项目及其所有者信息"""
        stmt = select(Project).options(joinedload(Project.owner)).where(Project.id == project_id)
    ...
    async def get_with_chapters(self, project_id: uuid.UUID) -> Project | None:
        """获取项目及其章节列表"""
        stmt = select(Project).options(selectinload(Project.chapters)).where(Project.id == project_id)
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def get_by_owner(self, owner_id: uuid.UUID, *, status: str | None = None) -> Sequence[Project]:
        """获取用户拥有的项目"""
    ...
    async def get_by_status(self, status: str, *, limit: int | None = None) -> Sequence[Project]:
        """根据状态获取项目"""
    ...
    async def update_progress(self, project_id: uuid.UUID, current_progress: int) -> Project | None:
        """更新项目进度"""
    ...
    async def increment_chapter_count(self, project_id: uuid.UUID) -> Project | None:
        """增加章节计数"""
    ...
    async def decrement_chapter_count(self, project_id: uuid.UUID) -> Project | None:
        """减少章节计数"""
        project = await self.get_by_id(project_id)
    ...
    async def search_by_name(self, name_query: str, *, limit: int = 20) -> Sequence[Project]:
        """根据名称搜索项目"""
        stmt = (
            select(Project)
...
class ProjectMemberRepository(BaseRepository[ProjectMember]):
    ...
    async def get_members_of_project(self, project_id: uuid.UUID) -> Sequence[ProjectMember]:
        """获取项目的所有成员"""
        stmt = (
    ...
    async def get_projects_of_user(self, user_id: uuid.UUID) -> Sequence[ProjectMember]:
        """获取用户参与的所有项目"""
        stmt = select(ProjectMember).options(joinedload(ProjectMember.project)).where(ProjectMember.user_id == user_id)
        result = await self._session.execute(stmt)
        return result.scalars().all()
    ...
    async def get_member_role(self, project_id: uuid.UUID, user_id: uuid.UUID) -> str | None:
        """获取用户在项目中的角色"""
    ...
    async def update_member_role(self, project_id: uuid.UUID, user_id: uuid.UUID, role: str) -> ProjectMember | None:
        """更新成员角色"""
